...
function createTree(arrName, startNode, openNode) {
	...
	if (nodes.length > 0) {
		...
		if (startNode !=0) {
			...
			document.write("<a href=\"" + nodeValues[3] + "\" target=\"rightfrm\" onmouseover=\"window.status='" + nodeValues[2] + "';return true;\" onmouseout=\"window.status=' ';return true;\"><img src=\"img/folderopen.gif\" align=\"absbottom\" alt=\"\" />" + nodeValues[2] + "</a><br />");
		} else document.write("<img src=\"img/base.gif\" align=\"absbottom\" alt=\"\" />学科分类<br />");
...
function addNode(parentNode, recursedNodes) {
	for (var i = 0; i < nodes.length; i++) {
		...
		if (nodeValues[1] == parentNode) {
			...
			if (hcn) {
				if (ls) {
					document.write("<a =\"javascript: oc(" + nodeValues[0] + ", 1);\"><img id=\"join" + nodeValues[0] + "\" src=\"img/");
			...
			// Start link
			document.write("<a href=\"" + nodeValues[3] + "\" target=\"rightfrm\"  onmouseover=\"window.status='" + nodeValues[2] + "';return true;\" onmouseout=\"window.status=' ';return true;\">");
			// Write out folder & page icons
			if (hcn) {
				document.write("<img id=\"icon" + nodeValues[0] + "\" src=\"img/folder")
					if (ino) document.write("open");
				document.write(".gif\" align=\"absbottom\" alt=\"\" />");
			} else document.write("<img id=\"icon" + nodeValues[0] + "\" src=\"img/page.gif\" align=\"absbottom\" alt=\"\" />");
